...
#            grads = opt.compute_gradients(loss) 
            ...
            sv = tf.train.Supervisor(is_chief=is_chief,
                                     logdir='resnet_my_method/saver1',
...
#            thread.start()
            ...
            # cthread = threading.Thread(target=check_read, args=(train_op))
            # cthread.start()
            ...
            for step in range(15):
                if step > 9 and FLAGS.job_name == 'worker' and FLAGS.task_id == 0:
                ...
                if step > 7 and FLAGS.job_name == 'worker' and FLAGS.task_id == 1:
                    ...
                    saver.save(sess, 'resnet_my_method/saver1/model.ckpt',global_step=global_step)
                    time.sleep(5)
                    command ="python resnet_my_method/method_1.py --job_name=worker --task_id=0 --ps_hosts='localhost:30000' --worker_hosts='localhost:30001'"
                    # print(command)
                    p = Process(target=change_PS, args=(command,))
                    p.start()
