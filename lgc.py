...
WORKER_FACTOR = (3-2)/3  ## (N-K)/N 1/number of faster workers
STRAGGLER_FACTOR = -1/3  ####1/N 
...
def loss_straggler(logits, labels, batch_size=FLAGS.batch_size):
    labels = tf.cast(labels, tf.int64)
    cross_entropy = tf.nn.sparse_softmax_cross_entropy_with_logits(
      logits=logits, labels=labels, name='cross_entropy_per_example')
    cross_entropy_mean = tf.reduce_mean(cross_entropy, name='cross_entropy')
    tf.add_to_collection('straggler_losses', cross_entropy_mean)

  # The total loss is defined as the cross entropy loss plus all of the weight
  # decay terms (L2 loss).
    return tf.add_n(tf.get_collection('straggler_losses'), name='total_loss')



def _add_loss_summaries(total_loss):
    loss_averages = tf.train.ExponentialMovingAverage(0.9, name='straggler_avg')
    losses = tf.get_collection('straggler_losses')
...
def _add_loss_summaries_worker(total_loss):
    loss_averages = tf.train.ExponentialMovingAverage(0.9, name='worker_avg')
    losses = tf.get_collection('worker_losses')
  ...
  # same for the averaged version of the losses.
    ...
    return loss_averages_op, loss_averages.average()
...
#            labels = tf.reshape(labels, [-1, _NUM_CLASSES])
            ...
            cross_entropy_dummy = tf.losses.softmax_cross_entropy(
                logits=logits, 
...
#            loss = cifar10.loss(logits, labels, batch_size)
            ...
            tf.add_to_collection('straggler_losses', cross_entropy)

            # tf.add_to_collection('straggler_losses', cross_entropy_dummy)

            loss = WORKER_FACTOR * cross_entropy + STRAGGLER_FACTOR * tf.add_n(
                [tf.nn.l2_loss(v) for v in tf.get_collection('straggler_losses')])
            ...
            # opt_test = tf.train.GradientDescentOptimizer(lr)

            # if FLAGS.job_name == 'worker' and (FLAGS.task_id == 2):
            #     # opt_for_straggler = tf.train.GradientDescentOptimizer(lr)
            #     # grad_for_straggler = opt_for_straggler.compute_gradients(loss)
            ...
            #     # opt = tf.train.GradientDescentOptimizer(lr)
            #         opt_for_straggler = tf.train.AdamOptimizer(lr)
    
            #         grad_for_straggler = opt_for_straggler.compute_gradients(loss)

            #         tf.add_to_collection('stragg_grad', grad_for_straggler)

            #         # straggler_grad[FLAGS.task_id] = grad_for_straggler

            #     variable_averages_straggler = tf.train.ExponentialMovingAverage(
            #         MOVING_AVERAGE_DECAY, global_step, name='strag_grad_avg')

            #     grads_all_straggler = tf.get_collection('stragg_grad')
            #     variables_averages_op_straggler = variable_averages_straggler.apply(grads_all_straggler)


            #     StragglerManager.straggler_avg_grad = variables_averages_op_straggler

            # apply_gradients_for_straggler = opt_for_straggler.apply_gradients(grad_for_straggler, global_step=global_step)

            # with tf.control_dependencies([apply_gradients_for_straggler]):
            #     train_op = tf.identity(loss, name='train_op')


            ########### separate  gradient descent optimiz
            ...
            opt = tf.train.SyncReplicasOptimizer(
                opt,
                replicas_to_aggregate=len(worker_hosts)-1,
...
#		grads.append((tf.scalar_mul(tf.cast(batch_size/FLAGS.batch_size, tf.float32), grad), var))
            ...
            # tf.add_to_collection('worker_losses', loss)

            # loss_averages = tf.train.ExponentialMovingAverage(0.9, name='ss_avg')
            # # losses = tf.get_collection('losses')
            # loss_averages_op, loss_averages_w = _add_loss_summaries_worker(loss)

            # losses_s = tf.get_collection('straggler_losses')
            # loss_averages_op_s = loss_averages.apply(losses_s)

            # # with tf.control_dependencies([loss_averages_op]):
            # #if StragglerManager.straggler_avg_loss is not None:
            # get_loss = loss_averages_w - loss_averages.average()
            ...
            # grad_test = opt_test.compute_gradients(loss)

            # grad_cast = tf.cast(grads0, tf.float32)
...
#            grads = opt.compute_gradients(loss) 
            ...
            # grad_tensor = tf.Variable(grads0, trainable=False)

            # for grad, var in grads0:
            #     tf.add_to_collection('grad_tesnor'+str(FLAGS.task_id), grad)

            # all_grad_tensor = tf.get_collection('grad_tensor'+str(FLAGS.task_id))

            # loss_averages_op = _add_loss_summaries_worker(loss)


            #  with tf.control_dependencies([loss_averages_op]):
            # # opt = tf.train.GradientDescentOptimizer(lr)
            #     opt_for_worker = tf.train.AdamOptimizer(lr)

            #     grad_for_worker = opt_for_straggler.compute_gradients(loss)

            # tf.add_to_collection('fastw_grad', grads0)

            # variable_averages_worker = tf.train.ExponentialMovingAverage(
            #         MOVING_AVERAGE_DECAY, global_step, name='worker_grad_avg')

            # # grads_all_worker = tf.convert_to_tensor(tf.get_collection('fastw_grad'), dtype = tf.float32)
            # grads_all_worker = tf.get_collection('fastw_grad')
            
            # grads_gv = []
            # for gv in tf.get_collection('fastw_grad'):
            #     grads_gv.append(gv)

            # average_grads = average_gradients(grads_gv)
            # variables_averages_op_worker = variable_averages_worker.apply(grads_all_worker)

            # with tf.control_dependencies([opt._variables_to_average]):
            # if StragglerManager.straggler_avg_grad is not None:
            #     e_t_1 = (StragglerManager.straggler_avg_grad - grads0)
...
#            thread.start()
            ...
            for step in range(25):
                ...
                # if step % 2 == 1:
                #     StragglerManager.worker_num = 2
                # else:
                #     StragglerManager.worker_num = 3
    ...
    #		    batch_size_num = 3600 + int(step/5)*50
                ...
                if FLAGS.job_name == 'worker' and FLAGS.task_id == 2 and StragglerManager.worker_num == 2:
                    time.sleep(3)
...
#                _, loss_value, gs = sess.run([train_op, loss, global_step], feed_dict={batch_size: batch_size_num}) 
    ...
    ##                    tf.logging.info("time statistics - batch_process_time: " + str( last_batch_time)  + " - train_time: " + str(b-start_time) + " - get_batch_time: " + str(c0-b) + " - get_bs_time:  " + str(c-c0) + " - accum_time: " + str(c-time0))
                    ...
                    # print("time: "+str(time.time())+ " step "+str(step)+" global step "+str(gs)+" loss "+str(loss_value)+" examples_per_sec "+str(examples_per_sec)+" sec_per_batch "+str(sec_per_batch)+
                    #     " duration "+str(duration)+" cpu_use "+str(cpu_use)+" mem use "+str(memoryUse)+" net "+str(net_usage/duration)+" cpu max "+str(cpu_max)+"mem max "+str(mem_max)
                    #     +" accuracy "+str(accuracy)+" number of workers "+str(StragglerManager.worker_num)+" val acc = "+str(val_accuracy))

                    print("step "+str(step)+" global_step "+str(gs)+ 
                        " train_acc "+str(accuracy)+" val_acc " + str(val_accuracy))
